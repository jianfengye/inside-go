...
var runtime_inittask initTask // 初始化任务，每个runtime的一些初始化需求

//go:linkname main_inittask main..inittask
var main_inittask initTask // main包的初始化需求
...
func main_main()  // 将这个函数变量链接到main.main中
...
// The main goroutine.
// 这个是runtime.main, g1是从gogo进到这个里面的
func main() {
	g := getg() // 这里是获取当前运行的goroutine
	...
	lockOSThread() // 锁住线程，不被其他m占用线程

	if g.m != &m0 {
		throw("runtime.main not on m0")
	}

	doInit(&runtime_inittask) // 做runtime级别的初始化任务 must be before defer
	if nanotime() == 0 {
	...
	doInit(&main_inittask) // 初始化main的init
...
func schedinit() {
	...
	sched.maxmcount = 10000 //最大的m数量

	tracebackinit() // 设置skipPC，调用堆栈初始化的元素
 	moduledataverify() // 模块数据验证
	stackinit() // 初始化堆栈
	mallocinit() // 分配内存堆
	mcommoninit(_g_.m) // 初始化m的结构
	cpuinit()       // must run before alginit
	alginit()       // 使用map前需要先调用这个 maps must not be used before this call
	modulesinit()   // 初始化go modules, provides activeModules
	typelinksinit() // uses maps, activeModules
	itabsinit()     // uses activeModules

	msigsave(_g_.m) // 将当前线程的信号屏蔽值放入m0的 sigmask
	initSigmask = _g_.m.sigmask

	goargs() // 存放所有的参数到全局的argslice中
	goenvs() // 存放命令行中的环境变量
	parsedebugvars() // 解析相关的调试变量
	gcinit() // 初始化gc
...
func mcommoninit(mp *m) {
	...
	mp.alllink = allm // 使用alllink可以查找到全局allm

	// NumCgoCall() iterates over allm w/o schedlock,
	// so we need to publish it safely.
	atomicstorep(unsafe.Pointer(&allm), unsafe.Pointer(mp)) // allm指向当前m
	unlock(&sched.lock)
...
func mstart1() {
	...
	if _g_.m == &m0 {
		mstartm0() //设置m0接收信号回调函数
	}

	// 创建m的时候会创建fn，大多数应该是nil
...
func execute(gp *g, inheritTime bool) {
	...
	casgstatus(gp, _Grunnable, _Grunning) // 修改g的状态和一些数值
	gp.waitsince = 0
	gp.preempt = false
...
top:
	...
	// 去本地p中获取runable q, local runq
	if gp, inheritTime := runqget(_p_); gp != nil {
		return gp, inheritTime
	}

	// 去全局队列获取runq global runq
	if sched.runqsize != 0 {
		lock(&sched.lock)
	...
	// 进行四轮
	for i := 0; i < 4; i++ {
		// 循环fastrand() % stealOrder.count次
		for enum := stealOrder.start(fastrand()); !enum.done(); enum.next() {
			if sched.gcwaiting != 0 {
				goto top
			}
			// 先去看runable, 再去看next, 前面两轮都是找runable，后面两轮找next
			stealRunNextG := i > 2 // first look for ready queues with more than 1 g
			// 去其他p偷goroutine，偷到了就返回
...
top:
	...
	if gp == nil {
		...
		// 查找全局的队列里面有没有可运行的g，每61次去全局可运行goroutine队列中获取
	...
	if gp == nil {
		// 试着去其他p中偷goroutine, 这个一定会找到才会返回的，如果没有找到，就继续走循环
	...
	if tryWakeP { // 只有gc偷的goroutine或者trace的才会启动p
		if atomic.Load(&sched.npidle) != 0 && atomic.Load(&sched.nmspinning) == 0 {
			wakep() // 启动p
		}
	}
...
func goexit1() {
	...
	mcall(goexit0) // 让调度器找一个m调用goexit0
}
...
func malg(stacksize int32) *g {
	newg := new(g)
	if stacksize >= 0 {
		stacksize = round2(_StackSystem + stacksize)  // 加上一些富裕buf大小，这个大小是用于处理信号量啥的
		systemstack(func() {
...
//go:nosplit
// 这里的siz为fn的参数个数，如果为main函数的话，siz就为0，fn为mainPC
func newproc(siz int32, fn *funcval) {
	//函数调用参数入栈顺序是从右向左，而且栈是从高地址向低地址增长的
	//注意：argp指向fn函数的第一个参数，而不是newproc函数的参数
	//参数fn在栈上的地址+8的位置存放的是fn函数的第一个参数
	argp := add(unsafe.Pointer(&fn), sys.PtrSize) // 跳过第一个指针，fn的第一个参数的地址
	gp := getg()  //获取当前g
	pc := getcallerpc() // 获取调用者的PC寄存器信息，就是调用者下一条指令要跑的地址
	// 在g0堆栈上跑newproc1
	systemstack(func() {
...
// argp: 执行函数的参数地址
// narg: 执行函数的参数个数
// callergp: 父goroutinue
// callerpc: 父goroutine调用的时候的程序运行位置
func newproc1(fn *funcval, argp unsafe.Pointer, narg int32, callergp *g, callerpc uintptr) {
	...
	siz := narg
	siz = (siz + 7) &^ 7  // 将size对齐8，siz是分配给goroutine的堆大小
	...
	gostartcallfn(&newg.sched, fn) // 设置newg的上下文
	newg.gopc = callerpc   // 保存调用方的pc
	newg.ancestors = saveAncestors(callergp) // 保存祖先信息
	newg.startpc = fn.fn // 保存startPC
	if _g_.m.curg != nil {
	...
	// 把新创建的g放入到当前的_p_的runnext上去，就是p0，因为这里可以确定当前的p没有要执行的goroutine
	runqput(_p_, newg, true)

	// g0进来的时候这里的mainStarted还为0，不能启动，schedt还没启动
	if atomic.Load(&sched.npidle) != 0 && atomic.Load(&sched.nmspinning) == 0 && mainStarted {
		wakep() // 唤醒当前的m
	}
	releasem(_g_.m) // 释放m的锁，和acquirem呼应
}
...
func (pp *p) init(id int32) {
	...
	pp.wbBuf.reset()
	// 给p设置mcache，要不然是已经分配的m的cache，要不然是重新申请
	if pp.mcache == nil {
		if id == 0 {
...
func procresize(nprocs int32) *p {
	...
	for i := nprocs - 1; i >= 0; i-- {
		p := allp[i]
		// 如果当前m的p，就不做其他操作，不进入空闲列表
		if _g_.m.p.ptr() == p {
			continue
		}
		p.status = _Pidle // 所有的p状态设置为idle
		if runqempty(p) { // 如果p上已经没有goroutine需要执行，将p放入schedt的空闲p列表中
			pidleput(p)
		} else {
			p.m.set(mget())
...
// Sched must be locked.
// 全局队列中获取goroutine，并且把最多一半的全局队列go都拿到本地来
func globrunqget(_p_ *p, max int32) *g {
	if sched.runqsize == 0 {
		return nil
	}
	// 计算这次我应该从全局队列中搬运的goroutine数量
	// 1 如果平均分，我应该分到的go数量
	// 2 必须小于max
	// 3 最多是队列的一半
	...
	gp := sched.runq.pop()  // 从队列中弹出一个goroutine作为返回值
	n--
	// 剩下n-1个全局队列的goroutine全部放到本地
	for ; n > 0; n-- {
		gp1 := sched.runq.pop()
		runqput(_p_, gp1, false) // 放入到当前p的可运行队列
	}
	return gp
}
...
// 判断p的runq队列是不是空的，如果是，返回true，如果不是，返回false
...
// 把gp放入到_p_上去，根据next判断存放到runable还是runnext中
...
// 从p中获取可以偷的go, 放到batch中，最多一般的runq，返回batch的个数
func runqgrab(_p_ *p, batch *[256]guintptr, batchHead uint32, stealRunNextG bool) uint32 {
	for {
		...
		// 确认下是否一致，如果tail和head此时变了，这里再更新
		if n > uint32(len(_p_.runq)/2) { // read inconsistent h and t
			continue
		}
		for i := uint32(0); i < n; i++ {
			// 把runq中的0-n的g都取出来，放到batch中
...
func runqsteal(_p_, p2 *p, stealRunNextG bool) *g {
	t := _p_.runqtail
	n := runqgrab(p2, &_p_.runq, t, stealRunNextG) // 去偷取p2最多一半的goroutine,存放到自己的runq中
	if n == 0 {
		return nil
	}
	n--
...
type initTask struct {
	...
	state uintptr // 状态 0 = uninitialized, 1 = in progress, 2 = done
	ndeps uintptr // 依赖包
	nfns  uintptr // init函数
...
func doInit(t *initTask) {
	...
	default: // not initialized yet
		...
		// 如果依赖包中还有依赖包，循环doInit
		for i := uintptr(0); i < t.ndeps; i++ {
			p := add(unsafe.Pointer(t), (3+i)*sys.PtrSize)
			t2 := *(**initTask)(p)
			doInit(t2)
		}
		// 对每个依赖包中的init函数进行一次调用
