...
// DI and SI.
// 在编译模式为c-archive或者c-share函数联结器会调用一次这个函数，将argc和argv放到DI,SI里面
...
TEXT runtime·rt0_go(SB),NOSPLIT,$0
	...
	LEAQ	(-64*1024+104)(SP), BX  //从系统线程的栈空分出一部分当作g0的栈，然后初始化g0的栈信息和stackgard
	...
	MOVL	$0, AX   // CPUID 指令的结果会存储到AX中
	CPUID
	MOVL	AX, SI
	CMPL	AX, $0  // 如果没有CPU信息，跳转
	JE	nocpuinfo
...
needtls:
	...
	get_tls(BX)  // 这里的get_tls是一个宏，在go_tls.h中定义。从 TLS(Thread Local Storage) 中，BX = &tls
	MOVQ	$0x123, g(BX) // 存储0x123到BX指向的tls中 tls[0] = 0x123
	MOVQ	runtime·m0+m_tls(SB), AX  // AX = m0.tls[0]
	CMPQ	AX, $0x123 // 通过检查 tls[0] 的值是否0x123来验证tls的功能是否正常
	JEQ 2(PC) //如果正常进入ok
	CALL	runtime·abort(SB) // 如果不正常的话
ok:
	...
	CLD				// 使方向标志为DF复位 convention is D is always left clearedXQ
	CALL	runtime·check(SB) // 检查当前操作系统各种类型的长度

	MOVL	16(SP), AX		// copy argc
	MOVL	AX, 0(SP)
	MOVQ	24(SP), AX		// copy argv
	MOVQ	AX, 8(SP)
	CALL	runtime·args(SB) // 检查参数，设置环境变量executable_path
	CALL	runtime·osinit(SB) // 设置ncpu, 设置pagesize
	CALL	runtime·schedinit(SB)   // 创建g1, 设置m，g值等

	// create a new goroutine to start program
	MOVQ	$runtime·mainPC(SB), AX		// 设置runtime.mainPC的入口
	PUSHQ	AX
	PUSHQ	$0			// arg size
	CALL	runtime·newproc(SB)  // 创建一个p结构挂载到m上
	POPQ	AX
	POPQ	AX

	// start this M
	CALL	runtime·mstart(SB)  // 启动调度

	CALL	runtime·abort(SB)	// 这个代码保证不会走到这里，走到这里就直接abort了。 mstart should never return
	RET
...
// 设置了全局变量，runtime.mainPC
// DATA 和 GLOBL是组合使用
// DATA    symbol+offset(SB)/width, value
// offset是相对于symbol的偏移量
...
TEXT runtime·gogo(SB), NOSPLIT, $16-8
	MOVQ	buf+0(FP), BX		// gobuf, fp是伪寄存器，存储参数和本地变量，相当于BX=&buf
	MOVQ	gobuf_g(BX), DX  // DX = buf.g
	MOVQ	0(DX), CX		// make sure g != nil
	get_tls(CX) //tls[0] = &g
	MOVQ	DX, g(CX) // CX = buf.g
	MOVQ	gobuf_sp(BX), SP	// SP = buf.sp restore SP
	MOVQ	gobuf_ret(BX), AX   // AX = buf.ret
	MOVQ	gobuf_ctxt(BX), DX  // DX = buf.ctxt
	MOVQ	gobuf_bp(BX), BP  // BP = buf.bp
	MOVQ	$0, gobuf_sp(BX)	// 重置buf值，clear to help garbage collector
	MOVQ	$0, gobuf_ret(BX)
	MOVQ	$0, gobuf_ctxt(BX)
	MOVQ	$0, gobuf_bp(BX)
	MOVQ	gobuf_pc(BX), BX // BX = buf.pc
	JMP	BX // 如果是g1的话，这里会跳转到runtime.mainPC
...
// 这个是普通的goroutine结束的时候会调用的方法
TEXT runtime·goexit(SB),NOSPLIT,$0-0
	BYTE	$0x90	// NOP
	CALL	runtime·goexit1(SB)	// 调用goexit1 does not return
